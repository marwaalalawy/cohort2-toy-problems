...
*/
 ...
 var buildTriangle = function(numOfRows,rows){

	if(!rows || rows.length === 0) return buildTriangle(numOfRows, [[1]]);
  	var newLine = [];
  	var lastLine = rows[rows.length - 1];
    ...
    newLine.push(1);
    rows.push(newLine);
    if(numOfRows === rows.length) {
    	return rows;
  	} else {
    	return buildTriangle(numOfRows, rows);
    }
}